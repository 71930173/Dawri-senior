...
const MyQueue = () => {
  ...
  const handleServeSpecific = async (appointmentId) => {
    ...
    try {
      const res = await staffAPI.serveSpecific(appointmentId);
      if (res.data?.success) {
  ...
  const handleMarkServed = async (appointmentId) => {
    ...
    try {
      const res = await staffAPI.markServed(appointmentId);
      if (res.data?.success) {
        toast.success(
          <div>
  ...
  const handleCancel = async (appointmentId, reason = '') => {
    try {
      const res = await staffAPI.cancelAppointment(appointmentId, reason);
      if (res.data?.success) {
  ...
  const handleResolveRemotely = async (appointmentId, note = '') => {
    ...
    try {
      const res = await staffAPI.resolveRemotely(appointmentId, note);
      if (res.data?.success) {
        toast.success(
          <div>
