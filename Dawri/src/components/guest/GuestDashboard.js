...
import {
  Calendar, Bell, TrendingUp, Clock,
  Users, ArrowRight, Ticket, MapPin, AlertTriangle,
  ChevronRight, Activity, Award, Zap, Heart, Crown
} from 'lucide-react';
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [activeRes, appointmentsRes, statsRes, notifRes] = await Promise.all([
        guestAPI.getActiveAppointment().catch(() => ({ data: null })),
        guestAPI.getMyAppointments().catch(() => ({ data: [] })),
        guestAPI.getStats().catch(() => ({ data: { total_visits: 0, completed: 0, avg_wait: 0 } })),
        guestAPI.getNotifications().catch(() => ({ data: [] }))
      ]);
  ...
  return (
    <div className="page-container pt-6 px-4 sm:px-6 lg:px-10 xl:px-16">
      ...
      <div className="max-w-6xl mx-auto">
        <PageHeader
          title={`${lang === 'ar' ? 'مرحباً' : 'Welcome'}, ${user?.firstName || user?.first_name || 'Guest'}!`}
          ...
          actions={
            ...
            >
              ...
              {lang === 'ar' ? 'الانضمام للطابور' : 'Join Queue'}
            </button>
          }
        />

        {/* Guest Priority Banner */}
        <div className="bg-accent-50 border-2 border-accent-300 rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="font-bold text-accent-700 text-lg">
              {lang === 'ar' ? 'أولوية الضيوف مفعلة' : 'Guest Priority Enabled'}
            </span>
          </div>
          <p className="text-accent-600">
            {lang === 'ar'
              ? 'كضيف، لديك أولوية (1) في الطابور أمام الطلاب (2). هذا يعني خدمة أسرع لك.'
              : 'As a guest, you have priority (1) in queue ahead of students (2). This means faster service for you.'}
          </p>
        </div>
              ...
              {activeAppointment ? (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => navigate(`/guest/queue-status/${activeAppointment.id}`)}
                    ...
                    <button
                      onClick={async () => {
                        if (window.confirm(lang === 'ar' ? 'هل أنت متأكد من إلغاء الطابور؟' : 'Are you sure you want to cancel?')) {
                          try {
                            await guestAPI.cancelAppointment(activeAppointment.id);
                            fetchDashboardData();
                          } catch (err) {
                ...
                <EmptyState
                  ...
                  description={lang === 'ar' ? 'ليس لديك موعد نشط حالياً. يمكنك إنشاء واحد الآن.' : 'You do not have an active appointment. Create one now.'}
                  action={
                    <button
                      onClick={() => navigate('/guest/queue')}
                      className="btn-primary gap-2"
            ...
            <AnimatedCard className="p-6">
              ...
              {notifications.length === 0 ? (
                ...
                <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                          notif.is_read ? 'bg-dark-300' : 'bg-accent-500'
                        }`} />
                  ...
                  <button
                    onClick={() => navigate('/guest/notifications')}
                    className="w-full py-2 text-sm text-accent-600 hover:text-accent-700 font-medium flex items-center justify-center gap-1 transition-colors"
                  >
                  ...
                  <tbody>
                    {appointments.map((appt) => (
                      <tr
                        ...
                        onClick={() => navigate(`/guest/queue-status/${appt.id}`)}
                      >
                        <td className="py-3 px-4">
                          <span className="font-bold text-accent-600">#{appt.ticket_number}</span>
                        </td>
