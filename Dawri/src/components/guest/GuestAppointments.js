...
const GuestAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      const response = await guestAPI.getMyAppointments();
  ...
  const handleCardClick = (appt) => {
    // Only open if status is waiting or serving
    if (appt.status === 'waiting' || appt.status === 'serving') {
      navigate(`/guest/queue-status/${appt.id}`);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white border-2 border-dark-200 rounded-xl focus:border-accent-500 focus:ring-4 focus:ring-accent-100 focus:outline-none transition-all"
            />
          </div>
        ...
        {filteredAppointments.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredAppointments.map((appt, index) => {
              const canOpen = appt.status === 'waiting' || appt.status === 'serving';
              ...
              return (
                ...
                >
                  <AnimatedCard
                    className={`p-5 ${!canOpen ? 'opacity-60' : ''}`}
                    onClick={() => handleCardClick(appt)}
                  >
                    <div className={`flex flex-col sm:flex-row sm:items-center gap-4 ${!canOpen ? 'cursor-default' : 'cursor-pointer'}`}>
                      {/* Ticket Number */}
                      <div className="w-16 h-16 rounded-2xl bg-accent-100 flex items-center justify-center flex-shrink-0">
                        <span className="text-xl font-bold text-accent-600">#{appt.ticket_number}</span>
                      </div>
